//! High-performance sharded counters with metriken integration.
//!
//! Provides [`Counter`] and [`CounterGroup`] for high-throughput metrics that
//! avoid cache-line contention by using per-thread shards.
//!
//! A [`CounterGroup`] provides sharded storage for up to 16 counters. Each
//! thread writes to its own shard (selected by thread ID), so counters in the
