...
fn build_config_from_saturate(
    ...
    if slo_p50.is_none() && slo_p99.is_none() && slo_p999.is_none() {
        return Err("saturate requires at least one SLO threshold (--slo-p50, --slo-p99, or --slo-p999)".into());
    }
...
fn parse_duration(s: &str) -> Result<Duration, String> {
    ...
    let value: u64 = num.parse().map_err(|e| format!("invalid number in duration: {e}"))?;
