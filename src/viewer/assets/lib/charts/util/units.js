...
const UNIT_SYSTEMS = {
    ...
    // Percentage (values are 0-1, multiply by 100 to get %)
    // Uses custom formatting in formatWithUnit for better precision near 100%
    percentage: {
        base: '%',
        scales: [{
            threshold: 0,
        ...
        custom: true  // Signal to use custom formatting
    },
...
function formatWithUnit(value, unitSystem, precision = 2) {
    ...
    // Special handling for percentages - show more precision near 100%
    if (normalizedUnitSystem === 'percentage') {
        const pctValue = value * 100;
        let formattedPct;
        if (pctValue >= 99.9) {
            // For values very close to 100%, show more decimal places
            formattedPct = pctValue.toFixed(3);
        } else if (pctValue >= 99) {
            formattedPct = pctValue.toFixed(2);
        } else {
            formattedPct = pctValue.toFixed(1);
        }
        // Remove trailing zeros
        formattedPct = formattedPct.replace(/\.0+$/, '').replace(/(\.\d*[1-9])0+$/, '$1');
        return { value: formattedPct, unit: '%' };
    }
